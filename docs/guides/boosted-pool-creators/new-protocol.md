...
The first consideration when integrating a new yield protocol into Boosted Pools is whether a new factory is even necessary. The ERC4626LinearPool is designed to be compatible with any ERC4626 compliant token vault, so they can be used directly (but don't forget to register a new [`protocolId`](./new-pool.md#protocol-ids)!)
...
This function returns the amount of assets (Main Token) that would be exchanged by the vault for the amount of shares (Wrapped Token) provided.

### `deposit`
```function deposit(uint256 assets, address receiver) public returns (uint256 shares)```

This function deposits assets of underlying tokens into the vault and grants ownership of shares to the receiver.
...
This function redeems a specific number of shares from the owner and sends assets of the underlying token (Main Token) from the vault to the receiver.
...
`_unwrapTokens`
This is equivalent to the withdraw function and will be used by the Rebalancer to remove tokens from yield-bearing vault.
...
## Token Interface
When changing the functions above, there will be a need to create an interface for the yield-bearing vault token (or even another contract from the protocol, see Aave code for example). Create this interface in the folder `/packages/interfaces/`. Notice that interface files and classes start with letter "I".

## Unit Tests
To create unit tests for your pool:
* In `packages/linear-pools/contracts/test`, create a Mock for each interface created in the interface folder in previous step. Use `MockERC4626Token.sol` as an example.
* To run tests, open folder `packages/linear-pools/` and run `yarn test`. Make sure it's passing before moving to the next step.
...
Fork tests are inside `/packages/deployment/tasks` folder. Create a folder with the format `YYYYMMDD-[linear-pool-folder-name]`, then create the following folders inside it:
* `build-info`
* `test`

Copy files `index.ts` , `input.ts` and `readme.md` from `20221116-erc4626-rebalanced-linear-pool` and copy it to your newly created task folder. Make sure to change `index.ts` and `input.ts` variables to adjust to your pool names. Also, copy the test file inside the test folder.
