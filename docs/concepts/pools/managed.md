...
### Examples
[ManagedPoolSetting.sol](https://github.com/balancer/balancer-v2-monorepo/blob/master/pkg/pool-weighted/contracts/managed/ManagedPoolSettings.sol) provides the necessary logic for viewing and updating token weights within a Managed Pool. how to can interact with token weights on a Managed Pool. Only an `owner` can call `updateWeightsGradually` but anyone can call the getter.
