...
<template>
  <div v-if="props.open" class="modal-container">
    <div class="modal-popup early-unlock">
      <h3 class="modal-title">Early Unlock</h3>
      <div class="body">
        <p class="text">You can enable or disable early unlock at any time</p>
      </div>
      <div class="btn-group">
        <button class="btn close" @click="props.onClose">Close</button>
        <button class="btn submit" @click="props.onUnlock">
          {{ props.earlyUnlock ? 'Disable' : 'Enable' }}
        </button>
      </div>
    </div>
  </div>
